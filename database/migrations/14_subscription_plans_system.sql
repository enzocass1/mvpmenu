/**
 * Migration 14: Subscription Plans System
 *
 * Crea sistema completo di gestione abbonamenti con:
 * - Piani di abbonamento flessibili
 * - Feature flags configurabili
 * - Super Admin panel
 * - CRM per gestione ristoranti
 * - Tracking eventi subscription
 * - Revenue analytics
 *
 * @version 14.0.0
 * @date 2025-10-27
...
CREATE INDEX idx_subscription_plans_visible ON subscription_plans(is_visible, is_active);
...
COMMENT ON COLUMN restaurants.subscription_status IS 'active, cancelled, expired, trial, suspended';
...
-- Event types
COMMENT ON COLUMN subscription_events.event_type IS 'subscription.created, subscription.upgraded, subscription.downgraded, subscription.cancelled, subscription.renewed, subscription.expired, payment.succeeded, payment.failed, trial.started, trial.ended';
...
CREATE INDEX idx_super_admin_logs_entity ON super_admin_logs(entity_type, entity_id);
...
-- Action types
COMMENT ON COLUMN super_admin_logs.action IS 'plan.created, plan.updated, plan.deleted, feature.created, restaurant.plan_assigned, super_admin.login, etc';
...
CREATE INDEX idx_revenue_analytics_month ON revenue_analytics(month DESC);
CREATE INDEX idx_revenue_analytics_plan ON revenue_analytics(plan_id);
CREATE INDEX idx_revenue_analytics_year ON revenue_analytics(year DESC);
...
-- Assegnare piano "Premium Legacy" a ristoranti con is_premium = true
UPDATE restaurants
SET
  subscription_plan_id = (SELECT id FROM subscription_plans WHERE slug = 'premium-legacy'),
  subscription_status = 'active',
  subscription_started_at = created_at,
  subscription_metadata = jsonb_build_object(
    'migrated_from', 'is_premium',
    'migration_date', NOW(),
    'original_is_premium', true
  )
WHERE is_premium = true;

-- Assegnare piano "Free" a ristoranti con is_premium = false
UPDATE restaurants
SET
...
-- Per ora disabilitato RLS su tabelle super admin
-- In produzione considerare di abilitare per sicurezza extra
...
-- Grant accesso alle tabelle (adjust based on your auth setup)
...
-- Super admin tables - restrict in production
...
BEGIN
  ...
  RAISE NOTICE '- 21 feature flags popolate';
