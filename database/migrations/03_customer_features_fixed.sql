-- ============================================
-- MIGRATION PART 3: Customer Features (FIXED)
-- ============================================
...
-- 3. Add missing columns to analytics_events table
DO $$
BEGIN
  -- Add customer_id
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'analytics_events' AND column_name = 'customer_id'
  ) THEN
    ALTER TABLE analytics_events ADD COLUMN customer_id UUID REFERENCES customers(id) ON DELETE SET NULL;
    COMMENT ON COLUMN analytics_events.customer_id IS 'ID del cliente (per eventi customer journey)';
  END IF;

  -- Add staff_id (senza FK perché staff table potrebbe non esistere)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'analytics_events' AND column_name = 'staff_id'
  ) THEN
    ALTER TABLE analytics_events ADD COLUMN staff_id UUID;
    COMMENT ON COLUMN analytics_events.staff_id IS 'ID dello staff che ha effettuato azione (no FK)';
  END IF;

  -- Add order_id
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'analytics_events' AND column_name = 'order_id'
  ) THEN
    ALTER TABLE analytics_events ADD COLUMN order_id UUID REFERENCES orders(id) ON DELETE SET NULL;
    COMMENT ON COLUMN analytics_events.order_id IS 'ID ordine relativo all evento';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE analytics_events ADD COLUMN product_id UUID REFERENCES products(id) ON DELETE SET NULL;
    COMMENT ON COLUMN analytics_events.product_id IS 'ID prodotto relativo all evento';
  END IF;
END $$;

-- 4. Enhanced Analytics Indexes (ONLY after customer_id exists)
...
-- 8. Function: Get or Create Anonymous Customer
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration PART 3 completata con successo!';
  RAISE NOTICE '- Loyalty tiers table creata';
  RAISE NOTICE '- customer_id aggiunto a orders';
  RAISE NOTICE '- customer_id aggiunto a analytics_events';
  RAISE NOTICE '- Indexes analytics ottimizzati';
...
UNION ALL
SELECT 'orders.customer_id' as info, COUNT(*)
FROM information_schema.columns
WHERE table_name = 'orders' AND column_name = 'customer_id'
UNION ALL
SELECT 'analytics_events.customer_id' as info, COUNT(*)
FROM information_schema.columns
WHERE table_name = 'analytics_events' AND column_name = 'customer_id';
