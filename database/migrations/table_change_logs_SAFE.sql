-- Table Change Logs - SAFE VERSION
...
DROP POLICY IF EXISTS "Restaurant owners can view table change logs" ON table_change_logs;
DROP POLICY IF EXISTS "Restaurant owners can insert table change logs" ON table_change_logs;
DROP POLICY IF EXISTS "Restaurant staff can view table change logs" ON table_change_logs;
DROP POLICY IF EXISTS "Restaurant staff can insert table change logs" ON table_change_logs;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE objoid = 'table_change_logs'::regclass
  ) THEN
